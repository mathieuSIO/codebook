...
export const ProductCard = ({ product }) => {
    ...
    const { id, name, overview, image_local, rating, price, poster, best_seller } = product;
        ...
        <div className="m-3 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            ...
            <div className="p-5">
                ...
                <p className="flex justify-between items-center">
                    ...
                    {inCart && <button onClick={() => removeFromCart(product)} className={`inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 disabled={product.in_stock ? "" : "disabled"}`} disabled={product.in_stock ? "" : "disabled"}>Remove Item <i className="ml-1 bi bi-trash3"></i></button>}
                </p>
            </div>
        </div>
    )
